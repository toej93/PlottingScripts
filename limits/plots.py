...
def beautify_efficiency(this_ax):
...
def beautify_veff(this_ax):
	...
	sizer=20
	#xlow = 3.e16 #the lower x limit
	xlow = np.power(10.,16.5)
	#xup = 3.e20 #the uppper x limit
	xup = np.power(10.,20.5)
	ylow = 1.e14 #the lower x limit
...
def beautify_counts(this_ax):
	...
	this_ax.tick_params(labelsize=sizer)
	this_ax.grid()
	this_legend = this_ax.legend(loc='upper left',title='Event Counts')
	setp(this_legend.get_texts(), fontsize=17)
	setp(this_legend.get_title(), fontsize=17)
